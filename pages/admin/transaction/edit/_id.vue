<template>
  <div class="admin-wrap">
    <div class="mode-editing">
      <TheBreadcrumb
        firstLink="/admin"
        firstText="Главная"
        :secondText="title"
      />
      <div class="item orange">Редактирование</div>
    </div>

    <h1 class="text-blue" v-if="isEditing">{{ title }}</h1>
...
export default {
  ...
  async asyncData({ store, params, req }) {
    ...
    let user = await store.dispatch('user/fetchByIdMinimal', authorData[0]._id)
    let forWhom = []

    forWhom.push(...user[0].subsNP, ...user[0].subsLE)

    for (let i = 0; i < forWhom.length; i++) {
      forWhom[i].label = forWhom[i].name
      forWhom[i].value = forWhom[i].telephone
      delete forWhom[i].name
      delete forWhom[i].telephone
    }
    let me = { label: 'Для себя', value: '' }
    forWhom.unshift(me)
    form.author = authorData[0]._id

    const label = forWhom.find((i) => i.value === post.forWhom.value)
    form.sub = label
      ...
      this.$refs.form.validate(async (valid) => {
        if (valid) {
          ...
          try {
            ...
            this.$refs.uploadPrototype.clearFiles()
            this.$router.push('/admin?message=created')
          } catch (e) {}
        } else {
